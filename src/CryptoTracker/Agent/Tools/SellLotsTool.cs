...
/// <summary>
/// Tool um Lots bei einem Fiat-Verkauf zuzuordnen.
/// </summary>
public sealed class SellLotsTool : IAgentTool
{
    ...
    public Delegate GetToolRunner() => SellLotsAsync;
    public string GetToolName() => "sell_lots";
    public string GetToolDescription() => """
        Ordnet Lots für einen Fiat-Verkauf zu.
        Parameter:
        - tradeId: ID des Sell-Trades
        - salePriceEurPerUnit: Verkaufspreis in EUR pro Einheit
        - allocationsJson: JSON-Array mit { lotId, quantity } oder leer/"AUTO_FIFO" für FIFO
        """;

    private async Task<string> SellLotsAsync(
        [Description("Trade ID")] int tradeId,
        [Description("Verkaufspreis EUR pro Einheit")] decimal salePriceEurPerUnit,
        [Description("JSON-Array {lotId, quantity}")] string allocationsJson)
    {
        ...
        {
            var autoResult = await BuildAutoFifoAllocationsForSellAsync(tradeId);
        ...
        var result = await _lotService.SellLotsAsync(tradeId, allocations, salePriceEurPerUnit);
        ...
        if (context != null)
        {
            ...
            {
                EventType = "assigned",
                Message = $"Verkauf zugeordnet: {result.TotalQuantity:F8} verkauft, Gewinn {result.TotalRealizedGain:F2} EUR",
    ...
    private async Task<AutoFifoResult> BuildAutoFifoAllocationsForSellAsync(int tradeId)
    {
